...
namespace Device
{
    ...
    public class Devices
    {
        ...
        /// 测温电桥设备
        /// </summary>
        public TempBridge tpBridge = new TempBridge();
        int tpBridgeReadCount = 0;
        ...
        public bool Configure(string configFilePath = @"./config.ini")
        {
            ...
            try
            {
                ...
                if (!File.Exists(configFilePath))
                {
                    ...
                    Utils.IniReadWrite.INIWriteValue(configFilePath, "PortName", "bridge", "COM5");

                    // 测温电桥参数
                    Utils.IniReadWrite.INIWriteValue(configFilePath, "TpBridge", "tpBridgeReadInterval", "3");
                    Utils.IniReadWrite.INIWriteValue(configFilePath, "TpBridge", "tpBridgeChannel", "3");
...
#if true
                // 测温电桥
                confOK &= tpBridge.SetDevicePortName(Utils.IniReadWrite.INIGetStringValue(configFilePath, "PortName", "bridge", "COM5"));
                tpBridge.tpBridgeReadInterval = int.Parse(Utils.IniReadWrite.INIGetStringValue(configFilePath, "TpBridge", "tpBridgeReadInterval", "3"));
                tpBridge.tpBridgeChannel = int.Parse(Utils.IniReadWrite.INIGetStringValue(configFilePath, "TpBridge", "tpBridgeChannel", "3"));
                Debug.WriteLineIf(!confOK, "配置测温电桥失败! 端口号: " + tpBridge.tpBridgePortName);
                Debug.WriteLineIf(confOK, "配置测温电桥成功! 端口号: " + tpBridge.tpBridgePortName);
                if (!confOK)
                    Utils.Logger.Sys("配置测温电桥失败! 端口号: " + tpBridge.tpBridgePortName);
#endif
        ...
        public bool DeviceSelfCheck()
        {
            ...
            // 传感器设备自检
            if (!tpBridge.SelfCheck())
            {
                checkOK = false;
                Debug.WriteLine("测温电桥自检失败!");
                Utils.Logger.Sys("测温电桥自检失败!");
            }
            else
            {
                Debug.WriteLine("测温电桥自检成功!");
            }
...
#region Timer Method
        ...
        private void TpTemperatureUpdateTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
...
#if DEBUG
            Console.WriteLine("tpDeviceM.temp = " + val.ToString());
#endif
...
#if DEBUG
            Console.WriteLine("tpDeviceS.temp = " + val.ToString());
#endif
            ...
            // 读取测温电桥值
            tpBridgeReadCount++;
            if (tpBridgeReadCount >= tpBridge.tpBridgeReadInterval)
            {
                tpBridgeReadCount = 0;
                Device.TempBridge.Err_b rt = tpBridge.UpdateBridgeValue();
                if (rt != Device.TempBridge.Err_b.NoError)
                {
                    //err = TempProtocol.Err_t.ComError;
                    // 如果发生错误，则直接触发事件，向主界面报错，并暂停流程控制
                    Debug.WriteLine("读取电桥温度时发生错误，errorCode: " + err.ToString());
                    Utils.Logger.Sys("读取电桥温度时发生错误，errorCode: " + err.ToString());
                    //err = TempProtocol.Err_t.NoError;
                    //goto End;
                }
                else
                {
                    if(tpBridge.temperatures.Count>0)
                    {
                        // 电桥温度实时记录
                        Utils.Logger.BridgeData(tpBridge.temperatures.Last());
                    }
                }
            }
#endif
...
#region Step Methods
        ...
        private void TempUpStep()
        {
            ...
            if (tpDeviceM.temperatures.Last() > currentState.tempPoint.stateTemp - 0.1)
            {
        ...
        private void TempControlStep()
        {
            ...
            if (steady)
            {
                ...
                currentState.flowState = State.TempStable;
                currentState.stateChanged = true;
                currentState.stateCounts = 0;
        ...
        private void TempStableStep()
        {
            ...
            if (currentState.stateCounts > bridgeSteadyTimeSec / tpDeviceM.readTempIntervalSec)
            {
                ...
                if (steady)
                {
                    ...
                    currentState.flowState = State.Measure;
                    currentState.stateChanged = true;
                    currentState.stateCounts = 0;
        ...
        private void MeasureStep()
        {
            ...
            Utils.Logger.Sys("温度点：" + currentState.tempPoint.stateTemp.ToString() + " 已经测量完成。");

            // 测量完成，标记
            temperaturePointList[currentState.tempPointIndex].finished = true;

            Utils.Logger.Sys("开始查找下一个温度点。");
            ...
            for (; i < temperaturePointList.Count; i++)
            {
                if (temperaturePointList[i].finished == false)
                {
                    ...
                    Utils.Logger.Sys("查找到了下一个未测量的温度点 " + currentState.tempPoint.stateTemp.ToString());
