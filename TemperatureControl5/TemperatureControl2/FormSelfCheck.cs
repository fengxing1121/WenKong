    ...
    public partial class FormSelfCheck : Form
    {
        ...
        private void BackgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            else
            {
                ...
                MessageBox.Show("继电器设备自检失败 - 总电源打开失败");
                goto ExitErr;
            }
            //Thread.Sleep(1500);
            ...
            for (int cmd = 1; cmd < 9; cmd++)
            {
                ...
                {
                    // 继电器设备自检失败
                    Utils.Logger.Sys("继电器设备自检失败 - 继电器打开失败");
                    MessageBox.Show("继电器设备自检失败 - 继电器打开失败！");
                    goto ExitErr;
                }
                Thread.Sleep(1500);
            }
            ...
            for (int cmd = 8; cmd > 0; cmd--)
            {
                ...
                {
                    // 继电器设备自检失败
                    Utils.Logger.Sys("继电器设备自检失败 - 继电器关闭失败");
                    MessageBox.Show("继电器设备自检失败 - 继电器关闭失败！");
                    goto ExitErr;
                }
                Thread.Sleep(1500);
            }
            ...
            if (deviceAll.tpBridge.SelfCheck() != true)
            {
                ...
                MessageBox.Show("测温电桥自检失败！");
                goto ExitErr;
            }
            Thread.Sleep(1500);
            this.BeginInvoke(new EventHandler(delegate
            {
                label2.Text = "测温电桥自检成功！";
            }));
            Thread.Sleep(1500);
            ...
            {
                // 继电器设备自检失败
                Utils.Logger.Sys("继电器设备自检失败 - 关闭总电源失败");
                MessageBox.Show("继电器设备自检失败 - 关闭总电源失败！");
                goto ExitErr;
            }
            this.BeginInvoke(new EventHandler(delegate
            {
